...
def main(fits_fn, outfn, nbits, apply_weights, apply_scales, apply_offsets):
    ...
    if nbits != 32:
        ...
        subint0 = psrfits_file.read_subint(
            0, apply_weights, apply_scales, apply_offsets
        )
    ...
    for isub in range(int(psrfits_file.nsubints)):
        subint = psrfits_file.read_subint(
            isub, apply_weights, apply_scales, apply_offsets
        )
        if flip_band:
...
if __name__ == "__main__":
    ...
    parser.add_option(
        ...
        default=8,
        type="int",
        help="The number of bits in the output .fil file. " + "Default=8",
    )
    parser.add_option(
        "-o",
        dest="outfn",
    ...
    main(
        ...
        outfn,
        options.nbits,
        options.apply_weights,
        options.apply_scales,
        options.apply_offsets,
    )
